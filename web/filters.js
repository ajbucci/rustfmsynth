...
export const sendSetFilter = async (opIndex, filterInt, filterParams) => {
...
export function createOperatorFilterManager(operatorIndex) {
  ...
  managerContainer.style.marginTop = '10px'; managerContainer.style.padding = '5px'; managerContainer.style.border = '1px solid lightgrey'; managerContainer.style.borderRadius = '5px';

  // 2. Active Filters Section Title (Unchanged)
  const activeFiltersTitle = document.createElement('h4');
  activeFiltersTitle.textContent = 'Active Filter Chain:';
  activeFiltersTitle.style.marginTop = '0'; activeFiltersTitle.style.marginBottom = '5px';
  ...
  // ----------------------------------------------------
  const adderTitle = document.createElement('h5');
  adderTitle.textContent = 'Add Filter:';
  adderTitle.style.marginTop = '15px'; adderTitle.style.borderTop = '1px solid #ccc'; adderTitle.style.paddingTop = '10px';
  ...
  defaultOption.value = ''; defaultOption.textContent = '-- Select Filter to Add --'; defaultOption.disabled = true; defaultOption.selected = true;
  ...
  adderParamsDiv.style.marginTop = '5px'; adderParamsDiv.style.padding = '5px';
...
function createActiveFilterDisplayInternal(operatorIndex, activeFilterData, displayArea, adderSelect, updateAdderOptionsCallback) {
  ...
  container.style.border = '1px solid #999'; container.style.borderRadius = '4px'; container.style.padding = '8px'; container.style.marginBottom = '5px';

  // --- Header setup (Unchanged) ---
  const header = document.createElement('div');
  header.style.display = 'flex'; header.style.justifyContent = 'space-between'; header.style.alignItems = 'center'; header.style.marginBottom = '5px'; header.style.borderBottom = '1px solid #eee'; header.style.paddingBottom = '3px';
  ...
  removeButton.textContent = '✕'; removeButton.title = `Remove ${filterConfig.name}`; removeButton.classList.add('remove-filter-button'); removeButton.dataset.operatorIndex = operatorIndex; removeButton.dataset.filterValue = activeFilterData.typeValue; removeButton.style.marginLeft = '10px'; removeButton.style.padding = '1px 5px'; removeButton.style.lineHeight = '1';
  ...
  removeButton.addEventListener('click', () => {
    ...
    console.log("--> External listener should now send 'remove_operator_filter' message.");
  });
  ...
  if (filterConfig.params?.length > 0) {
    filterConfig.params.forEach(paramInfo => {
      ...
      if (inputElement) {
        inputElement.addEventListener('change', async (event) => { // *** Added async ***
          ...
          const filtVal = parseInt(container.dataset.filterValue);

          if (isNaN(opIdx) || isNaN(filtVal)) {
    ...
    const nt = document.createElement('span'); nt.textContent = "No params."; nt.style.fontStyle = "italic";
