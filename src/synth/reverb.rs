...
fn main() {
    ...
    // RNG no longer needed for reverb creation/configuration

    let num_internal_channels = 8;
    let diffusion_steps = 4;
    let room_size_ms = 80.0;
    let rt60_seconds = 2.5;
    let dry_wet_mix = 0.4;
    ...
    reverb.configure(sample_rate); // Configure requires only sample rate now

    let buffer_size = 512;
    let num_buffers = (sample_rate * (rt60_seconds + 1.0)) as usize / buffer_size;
    let mut input_signal = vec![0.0f32; num_buffers * buffer_size];
    input_signal[0] = 0.8;
    let mut output_signal = vec![0.0f32; num_buffers * buffer_size];

    println!("Processing buffer-by-buffer (RT60={})...", rt60_seconds);
    for i in 0..num_buffers {
        let start = i * buffer_size;
        let end = start + buffer_size;
        output_signal[start..end].copy_from_slice(&input_signal[start..end]);
        reverb.process(&mut output_signal[start..end]);
    }
    println!("Processing done.");

    let tail_start_index = output_signal.len().saturating_sub(50);
    let tail_end = &output_signal[tail_start_index..];
    println!("Output Tail (RT60={}):", rt60_seconds);
    let max_abs_tail = tail_end.iter().map(|x| x.abs()).fold(0.0f32, f32::max);
    println!("Max absolute value in tail: {:e}", max_abs_tail);
    // Expect non-zero tail for f32, check if it's reasonably small
    if max_abs_tail < 1e-6 {
        println!("Info: Reverb tail decayed to < 1e-6.");
    } else {
        println!("Info: Reverb tail max value is {:e}.", max_abs_tail);
    }
}
